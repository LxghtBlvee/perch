...
<template>
  <div class="p-6 space-y-8 max-w-2xl">
    <div>
      ...
      <p class="text-sm text-muted-foreground mt-1">Your account and preferences.</p>
    </div>

    <!-- Appearance -->
    <section class="space-y-3">
      <h2 class="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Appearance</h2>
    ...
    <!-- Profile -->
    <section class="space-y-3">
      <h2 class="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Profile</h2>
      <div class="rounded-xl border border-border bg-card p-5 space-y-4">
        <div class="flex items-center gap-4">
          <div class="size-12 rounded-full bg-primary/10 flex items-center justify-center shrink-0 overflow-hidden">
            <img v-if="auth.user?.avatarUrl" :src="auth.user.avatarUrl" class="size-12 object-cover">
            <span v-else class="text-lg font-semibold text-primary">
    ...
    <section v-if="auth.user?.hasPassword && !auth.user?.seeded" class="space-y-3">
      <h2 class="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Password</h2>
      <div class="rounded-xl border border-border bg-card p-5 space-y-4">
        <div class="space-y-1">
          <label class="text-xs text-muted-foreground">Current password</label>
          <div class="relative">
            <input v-model="currentPassword" :type="showCurrent ? 'text' : 'password'" placeholder="••••••••" :class="INPUT + ' pr-10'">
            <button class="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" @click="showCurrent = !showCurrent">
              <Eye v-if="!showCurrent" class="size-4" :stroke-width="1.75" />
              <EyeOff v-else class="size-4" :stroke-width="1.75" />
            </button>
          </div>
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div class="space-y-1">
            ...
            <div class="relative">
              <input v-model="newPassword" :type="showNew ? 'text' : 'password'" placeholder="••••••••" :class="INPUT + ' pr-10'">
              <button class="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" @click="showNew = !showNew">
                <Eye v-if="!showNew" class="size-4" :stroke-width="1.75" />
                <EyeOff v-else class="size-4" :stroke-width="1.75" />
              </button>
            </div>
          </div>
          <div class="space-y-1">
            ...
            <div class="relative">
              ...
              <Check v-if="confirmPassword && confirmPassword === newPassword" class="absolute right-2.5 top-1/2 -translate-y-1/2 size-4 text-green-500 pointer-events-none" :stroke-width="2" />
            </div>
          </div>
        </div>
        <button
